...
template<typename T>
size_t read_problem_info(const char* train_file, bool read_cache,
		size_t& line_cnt, size_t num_threads = 0);
...
T calc_loss(T y, T pred) {
	pred = std::max(std::min(pred, (T)MAX_SIGMOID), (T)MIN_SIGMOID);
	T loss = y > 0 ? -log(pred) : -log((T)1. - pred);
	return loss;
}
...
public:
	...
	bool Train(const char* last_model, const char* model_file,
		const char* train_file, const char* test_file = NULL);

protected:
	bool TrainImpl(const char* model_file, const char* train_file,
		size_t line_cnt, const char* test_file = NULL);
...
template<typename T>
bool FtrlTrainer<T>::Train(T alpha, T beta, T l1, T l2, T dropout,
		const char* model_file, const char* train_file,
...
template<typename T>
bool FtrlTrainer<T>::Train(const char* last_model, const char* model_file,
...
template<typename T>
bool FtrlTrainer<T>::TrainImpl(const char* model_file, const char* train_file,
...
template<typename T>
size_t read_problem_info(const char* train_file, bool read_cache,
		size_t& line_cnt, size_t num_threads) {
	size_t feat_num = 0;
	line_cnt = 0;
...
template<typename T>
bool FastFtrlTrainer<T>::Train(T alpha, T beta, T l1, T l2, T dropout,
		const char* model_file, const char* train_file,
		const char* test_file) {
	if (!init_) return false;
...
template<typename T>
bool FastFtrlTrainer<T>::Train(const char* last_model, const char* model_file,
		const char* train_file, const char* test_file) {
	if (!init_) return false;
...
template<typename T>
bool FastFtrlTrainer<T>::TrainImpl(const char* model_file, const char* train_file,
		size_t line_cnt, const char* test_file) {
	if (!init_) return false;
